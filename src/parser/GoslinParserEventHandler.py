...
class GoslinParserEventHandler(BaseParserEventHandler):
    ...
    def new_fa(self, node):
        if self.level == LipidLevel.SPECIES:
            self.current_fa = StructuralFattyAcid()
            
        elif self.level == LipidLevel.MOLECULAR_SUBSPECIES:
            self.current_fa = MolecularFattyAcid("FA%i" % (len(self.fa_list) + 1), 2, 0, 0, LipidFaBondType.ESTER, False, len(self.fa_list) + 1)
            
        elif self.level == LipidLevel.STRUCTURAL_SUBSPECIES:
            self.current_fa = StructuralFattyAcid("FA%i" % (len(self.fa_list) + 1), 2, 0, 0, LipidFaBondType.ESTER, False, len(self.fa_list) + 1)
    ...
    def build_lipid(self, node):
        ...
        if self.level == LipidLevel.SPECIES:
            ...
            self.current_fa = LipidMolecularSubspecies(self.head_group, self.fa_list[0])
